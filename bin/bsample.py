...
采集 LSF 集群数据并存入 SQLite 数据库，供 bmonitor GUI 和 bmonitor_cli 查询。
支持同时指定多个采样项（并行执行），至少需指定一项。

数据存储在 config_lsf.py 的 db_path 下（为空时回退到 config.py 的 db_path/lsf，
默认 <install>/db/lsf/<cluster>/）。
""",
...
使用示例：
  ...
  # 组合采样（同时执行多项）
  bsample -j -m -q -qH -gH -H -l    # 常规全量采样（不含慢查询）
  bsample -j -m -q -H -l            # 轻量快速采样
  bsample -U -UD                    # 利用率 + 日利用率统计
  bsample -u                        # 用户作业统计

  # 清理过期数据
  bsample -c                        # 按 cleanup_expire_days 清理旧数据

  # 生成 AI 集群分析报告（需配置 AI）
  bsample -A

推荐 crontab 配置：
  */5  * * * * bsample -m -q -H -l                 # 高频：内存/队列/主机/负载
  */30 * * * * bsample -qH -gH                     # 中频：队列-主机/group-主机映射
  10   11,23 * * * bsample -j                      # 低频：已完成作业
  30   11,23 * * * bsample -u                      # 低频：用户统计
  */10 * * * * bsample -U                          # 中频：利用率
  55   23 * * * bsample -UD                        # 每日：日利用率
  3    0 * * * bsample -c                          # 每日：清理
  5    8 * * * bsample -A                          # 每日：AI 分析报告
""",
    )
    ...
    parser.add_argument("-c", "--cleanup",
                        action="store_true",
                        default=False,
                        help='清理过期数据（按 config_lsf.py 的 cleanup_expire_days 配置）')
    parser.add_argument("-j", "--job",
                        action="store_true",
                        default=False,
                        help='采集已完成作业信息（bjobs -u all -d -UF）')
    parser.add_argument("-m", "--job_mem",
                        action="store_true",
                        default=False,
                        help='采集运行中作业的内存和 idle_factor（bjobs -o）')
    ...
    parser.add_argument("-qH", "--queue_host_mapping",
                        action="store_true",
                        default=False,
                        help='采集队列-主机映射（bqueues -l）')
    parser.add_argument("-gH", "--group_host_mapping",
                        action="store_true",
                        default=False,
                        help='采集 host group-主机映射（bmgroup -w -r）')
    ...
    parser.add_argument("-l", "--load",
                        action="store_true",
                        default=False,
                        help='采集主机负载 ut/tmp/swp/mem（lsload）')
    parser.add_argument("-u", "--user",
                        action="store_true",
                        default=False,
                        help='采集用户作业统计（bjobs -u all -d -UF）')
    parser.add_argument("-U", "--utilization",
                        action="store_true",
                        default=False,
                        help='采集集群利用率 slot/cpu/mem（lsload/bhosts/lshosts）')
    parser.add_argument("-UD", "--utilization_day",
                        action="store_true",
                        default=False,
                        help='计算并保存日利用率统计（基于 -U 数据）')
    parser.add_argument("-A", "--analyze",
                        action="store_true",
                        default=False,
                        help='生成 AI 集群分析 HTML 报告（需配置 config_ai.py）')

    args = parser.parse_args()

    if not any([args.cleanup, args.job, args.job_mem, args.queue, args.queue_host_mapping, args.group_host_mapping, args.host, args.load, args.user, args.utilization, args.utilization_day, args.analyze]):
        common.bprint('At least one argument of "cleanup/job/job_mem/queue/queue_host_mapping/group_host_mapping/host/load/user/utilization/utilization_day/analyze" must be selected.', level='Error')
        sys.exit(1)

    return args.cleanup, args.job, args.job_mem, args.queue, args.queue_host_mapping, args.group_host_mapping, args.host, args.load, args.user, args.utilization, args.utilization_day, args.analyze
...
class Sampling:
    ...
    """
    def __init__(self, cleanup, job_sampling, job_mem_sampling, queue_sampling, queue_host_mapping_sampling, group_host_mapping_sampling, host_sampling, load_sampling, user_sampling, utilization_sampling, utilization_day_sampling, analyze_sampling):
        ...
        self.analyze_sampling = analyze_sampling
        ...
        global config_lsf
        self.db_path = common_db_path.resolve_db_path(config_lsf, 'lsf')
        (self.tool, cluster) = self.check_cluster_info()
        self.cluster = cluster

        # Reload cluster-specific config_lsf if exists; rebind the module-level
        # config_lsf so downstream reads (db_path, cleanup_expire_days, lmstat_*)
        # pick up the cluster-specific overrides.
        reloaded = common_config.reload_config_for_cluster(cluster)

        if reloaded is not None:
            config_lsf = reloaded

        if cluster:
            self.db_path = common_db_path.resolve_db_path(config_lsf, 'lsf') + '/' + str(cluster)

        # Data retention days for cleanup (after cluster config_lsf reload).
        ...
        if hasattr(config_lsf, 'cleanup_expire_days') and isinstance(config_lsf.cleanup_expire_days, dict):
            default_cleanup_expire_days.update(config_lsf.cleanup_expire_days)
        ...
        # Sampling data is written by the dedicated sampler (owner) and only
        # read by others via the GUI, so these dirs are 0o755 (owner-writable).
        common.create_dir(self.db_path, 0o755)
        common.create_dir(self.job_db_path, 0o755)
        common.create_dir(self.job_data_db_path, 0o755)
        common.create_dir(self.user_db_path, 0o755)
    ...
    def cleanup_db(self):
        ...
        # Sweep stale/orphaned malformed-suspect counter files left over from
        # the self-heal mechanism (db files deleted by -c, or ranges that
        # stopped sampling). Counter dir lives next to job_data dbs.
        try:
            removed = common_sqlite3.cleanup_suspect_counters(self.job_data_db_path)

            if removed > 0:
                common.bprint(f'Removed {removed} stale malformed-suspect counter file(s) under "{self.job_data_db_path}".', date_format='%Y-%m-%d %H:%M:%S')
        except Exception as error:
            common.bprint(f'*Warning* Failed to sweep suspect counters under "{self.job_data_db_path}": {error}', date_format='%Y-%m-%d %H:%M:%S', level='Warning')
    ...
    def _cleanup_date_dir(self, dir_path, item_name):
        ...
        expire_days = self.cleanup_expire_days.get(item_name, 30)
    ...
    def _cleanup_job_data_db(self):
        ...
        expire_days = self.cleanup_expire_days.get('job_data', 30)
    ...
    def sample_job_mem_info(self):
        ...
        job_range_dic = common.get_job_range_dic(job_list, range_size=100000)
    ...
    def sample_queue_info(self):
        ...
        if result == 'passed':
            try:
                ...
                queue_list = bqueues_dic.get('QUEUE_NAME', []) + ['ALL']
    ...
    def sample_user_info(self):
        ...
        Sample user info.
        """
        common.bprint('>>> Sampling user info ...', date_format='%Y-%m-%d %H:%M:%S')
        common.bprint('* Getting finished job information with command "bjobs -u all -d -UF" ...', date_format='%Y-%m-%d %H:%M:%S', indent=4)
        bjobs_dic = common_lsf.get_bjobs_uf_info('bjobs -u all -d -UF')
            ...
            try:
                ...
                for utilization_table_name in utilization_table_list:
                    ...
                    if utilization_db_data_dic:
                        ...
                        for i in range(len(utilization_db_data_dic['slot'])):
    ...
    def sample_cluster_analyze(self):
        """
        Generate an AI cluster analyze HTML report and save it (timestamped) under
        <db_path>/ai_report/.
        """
        common.bprint('>>> Generating AI cluster analyze report ...', date_format='%Y-%m-%d %H:%M:%S')

        # Require AI configuration (from config_ai.py, not config_lsf.py).
        if not (getattr(config_ai, 'ai_api_base_url', '') and getattr(config_ai, 'ai_api_key', '') and getattr(config_ai, 'ai_model_name', '')):
            common.bprint('AI is not configured (ai_api_base_url/ai_api_key/ai_model_name in config_ai.py), skip cluster analyze.', date_format='%Y-%m-%d %H:%M:%S', level='Warning', indent=4)
            return

        try:
            from common import common_ai
        except Exception as error:
            common.bprint(f'Failed to import common_ai for cluster analyze: {error}', date_format='%Y-%m-%d %H:%M:%S', level='Warning', indent=4)
            return
        ...
        try:
            ...
            content = common_ai.generate_cluster_analyze_report(
                api_base_url=config_ai.ai_api_base_url,
                api_key=config_ai.ai_api_key,
                model_name=config_ai.ai_model_name,
                tool=self.tool,
                db_path=self.db_path,
                lmstat_path=getattr(config_lsf, 'lmstat_path', 'lmstat'),
                lmstat_bsub_command=getattr(config_lsf, 'lmstat_bsub_command', ''),
                doc_chunks=doc_chunks,
                embedding_model=getattr(config_lsf, 'ai_embedding_model_name', ''),
                embedding_api_base_url=getattr(config_lsf, 'ai_embedding_api_base_url', ''),
                embedding_api_key=getattr(config_lsf, 'ai_embedding_api_key', ''),
            )
            ...
            report_dir = common_ai.resolve_report_dir('cluster')
            report_file = os.path.join(report_dir, f'{self.cluster}_{self.sample_time}.html')

            with open(report_file, 'w', encoding='utf-8') as RF:
                RF.write(html)

            common.bprint(f'Cluster analyze report saved: {report_file}', date_format='%Y-%m-%d %H:%M:%S', indent=4)
        except Exception as error:
            common.bprint(f'Failed on generating cluster analyze report: {error}', date_format='%Y-%m-%d %H:%M:%S', level='Warning', indent=4)
    ...
    def sampling(self):
        ...
        # AI cluster analyze is a single (slow) LLM call; run it inline after the
        # parallel samplers so its output and errors are visible.
        if self.analyze_sampling:
            self.sample_cluster_analyze()
...
#################
def main():
    (cleanup, job, job_mem, queue, queue_host_mapping, group_host_mapping, host, load, user, utilization, utilization_day, analyze) = read_args()

    # Ensure db_path root is 0o1777 before Sampling.__init__ creates lsf/
    # subdirs under it. For a custom db_path created on first run, makedirs
    # would otherwise leave the root at 0o755 (umask) and lock out other
    # sampler accounts from creating their own subsystem subdirs.
    try:
        lsf_root = common_db_path.resolve_db_path(config_lsf, 'lsf')
        common_db_path.ensure_db_root(os.path.dirname(lsf_root))
    except Exception:
        pass

    my_sampling = Sampling(cleanup, job, job_mem, queue, queue_host_mapping, group_host_mapping, host, load, user, utilization, utilization_day, analyze)
