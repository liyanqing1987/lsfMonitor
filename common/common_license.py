...
class GetLicenseInfo():
    ...
    """
    def __init__(self, specified_servers=None, specified_feature='', lmstat_path='lmstat', bsub_command='bsub -q normal -Is'):
        if specified_servers is None:
            specified_servers = []
        ...
        if specified_servers:
            server_list = os.environ.get('LM_LICENSE_FILE', '').split(':')
    ...
    def get_license_info(self):
        ...
        if 'LM_LICENSE_FILE' in os.environ:
            stdout_list = []
            lm_license_file_list = os.environ.get('LM_LICENSE_FILE', '').split(':')
            ...
            with ProcessPoolExecutor(max_workers=min(len(lm_license_file_list), 16)) as executor:
                ...
                try:
                    for job in as_completed(job_list, timeout=120):
                        try:
                            (return_code, stdout, stderr) = job.result()

                            if isinstance(stdout, bytes):
                                stdout_list.extend(stdout.decode('utf-8', errors='replace').split('\n'))
                        except Exception as error:
                            common.bprint(f'lmstat job failed: {error}', level='Warning')
                except TimeoutError:
                    common.bprint('Some lmstat jobs timed out after 120s, results may be incomplete.', level='Warning')

                    for pending_job in job_list:
                        pending_job.cancel()
        else:
            lmstat_command = self.get_lmstat_command()
            (return_code, stdout, stderr) = common.run_command(lmstat_command)
            stdout_list = stdout.decode('utf-8', errors='replace').split('\n')
...
def match_exact_or_fuzzy(keywords, items):
    ...
    Thin wrapper around common.match_exact_or_fuzzy (kept here so existing
    common_license.match_exact_or_fuzzy callers keep working).
    """
    return common.match_exact_or_fuzzy(keywords, items)
...
class FilterLicenseDic():
    ...
    def filter_by_feature(self, license_dic, feature_list):
        ...
        Each keyword is matched exact-preferred (exact match wins; fuzzy only
        when no exact match). 'ALL' means no filter.
        """
        if 'ALL' in feature_list:
            return license_dic

        # Collect all feature names, then match exact-preferred.
        all_feature_list = []

        for license_server in license_dic.keys():
            for vendor_daemon in license_dic[license_server]['vendor_daemon'].keys():
                for feature in license_dic[license_server]['vendor_daemon'][vendor_daemon]['feature'].keys():
                    if feature not in all_feature_list:
                        all_feature_list.append(feature)

        filtered_feature_list = match_exact_or_fuzzy(feature_list, all_feature_list)
    ...
    def filter_by_feature_usage_attribute(self, license_dic, feature_usage_attribute, feature_usage_attribute_value_list):
        ...
        when no exact match). 'ALL' means no filter.
        """
        if 'ALL' in feature_usage_attribute_value_list:
            return license_dic

        # Collect all values for this attribute, then match exact-preferred.
        all_value_list = []
            ...
            for vendor_daemon in license_dic[license_server]['vendor_daemon'].keys():
                for feature in license_dic[license_server]['vendor_daemon'][vendor_daemon]['feature'].keys():
                    for (i, usage_dic) in enumerate(license_dic[license_server]['vendor_daemon'][vendor_daemon]['feature'][feature]['in_use_info']):
                        if usage_dic[feature_usage_attribute] not in all_value_list:
                            all_value_list.append(usage_dic[feature_usage_attribute])

        filtered_feature_usage_attribute_value_list = match_exact_or_fuzzy(feature_usage_attribute_value_list, all_value_list)
    ...
    def filter_show_mode_feature(self, license_dic, show_mode):
        ...
        show_mode can be a single mode string OR a list of mode strings (for
        multi-select combos). When it's a list, a record is kept if it matches
        ANY of the selected modes (OR semantics). Modes: "IN_USE/NOT_USED" or
        "Expired/Nearly_Expired/Unexpired". 'ALL' / empty means no filtering.
        """
        # Normalize to a list of modes; drop ALL / empties (they mean no filter).
        if isinstance(show_mode, str):
            mode_list = [show_mode]
        else:
            mode_list = list(show_mode)

        mode_list = [m for m in mode_list if m and m != 'ALL']
        ...
        for license_server in license_dic.keys():
            for vendor_daemon in license_dic[license_server]['vendor_daemon'].keys():
                for feature in license_dic[license_server]['vendor_daemon'][vendor_daemon]['feature'].keys():
                    ...
                    if any(m in ['IN_USE', 'NOT_USED'] for m in mode_list):
                        in_use = license_dic[license_server]['vendor_daemon'][vendor_daemon]['feature'][feature]['in_use']

                        # Keep the feature's expires records only if the feature
                        # matches at least one selected IN_USE/NOT_USED mode.
                        if (('IN_USE' in mode_list) and (in_use != '0')) or (('NOT_USED' in mode_list) and (in_use == '0')):
                            if feature in license_dic[license_server]['vendor_daemon'][vendor_daemon]['expires']:
                            ...
                            continue
                    elif any(m in ['Expired', 'Nearly_Expired', 'Unexpired'] for m in mode_list):
                        if feature in license_dic[license_server]['vendor_daemon'][vendor_daemon]['expires']:
                            for expire_dic in license_dic[license_server]['vendor_daemon'][vendor_daemon]['expires'][feature]:
                                expire_mark = check_expire_date(expire_dic['expires'])

                                # OR semantics: keep if it matches any selected mode.
                                keep = False

                                if ('Expired' in mode_list) and (expire_mark == -1):
                                    keep = True
                                elif ('Nearly_Expired' in mode_list) and (expire_mark != -1) and (expire_mark != 0):
                                    keep = True
                                elif ('Unexpired' in mode_list) and (expire_mark == 0):
                                    keep = True
    ...
    def run(self, license_dic, server_list=None, vendor_list=None, feature_list=None, submit_host_list=None, execute_host_list=None, user_list=None, show_mode='ALL'):
        """
        ...
        if submit_host_list is None:
            submit_host_list = []

        if execute_host_list is None:
            execute_host_list = []
        ...
        # show_mode may be a single string (legacy) or a list (multi-select).
        # Filter only when there's at least one non-ALL mode selected.
        if isinstance(show_mode, str):
            do_filter = show_mode and show_mode != 'ALL'
        else:
            do_filter = any(m and m != 'ALL' for m in show_mode)
