...
def get_bhosts_load_info(command='bhosts -l'):
    ...
    # Sanity check + whole-fallback: bhosts -l parses Total rows by column
    # index alignment (head_list[i] <-> total_load_list[i]); if any host's
    # output has fewer/more columns (e.g. an empty 'it' field collapses split),
    # values shift and 'ut' may capture 'r1m'/'r15m' (run-queue length, can be
    # far >100). A single >100 ut means the whole parse is suspect (same code
    # path for every host), so discard bhosts_load entirely and rebuild from
    # 'lsload' (one call, all hosts) so all columns stay consistent.
    if _bhosts_load_ut_has_outlier(bhosts_load_dic):
        common.bprint('bhosts -l ut sanity check failed (some ut >100, likely column misalignment), falling back to lsload for all hosts.', level='Warning')

        lsload_dic = get_lsload_info()

        return _build_bhosts_load_from_lsload(lsload_dic)

    return bhosts_load_dic


def _bhosts_load_ut_has_outlier(bhosts_load_dic):
    """Return True if any host's parsed 'ut' is >100 or non-numeric (suspect)."""
    for host in bhosts_load_dic:
        total = bhosts_load_dic[host].get('Total', {})
...
def _build_bhosts_load_from_lsload(lsload_dic):
    """Rebuild a bhosts_load-shaped dict from lsload data (Total level only,
    no Reserved — lsload has no reserved row).

    Used as a whole-fallback when bhosts -l ut sanity check fails, so all
    columns (ut/mem/...) stay consistent (from the same source) instead of a
    mix of bhosts -l (some hosts) + lsload (outlier hosts). Downstream reads
    bhosts_load_dic[host]['Total']['ut'/'mem'/...], so the shape is preserved.
    """
    new_dic = {}

    if not lsload_dic or 'HOST_NAME' not in lsload_dic:
        return new_dic

    host_names = lsload_dic['HOST_NAME']

    # lsload_dic is {column_name: [values]}; map matching keys into the
    # bhosts_load 'Total' shape. ut is normalized (drop '%' and '*' high-load
    # marker) so the downstream '^[0-9]+$' check doesn't trip on a '*'.
    for idx, host in enumerate(host_names):
        total = {}

        for key in ('r15s', 'r1m', 'r15m', 'ut', 'pg', 'ls', 'it', 'tmp', 'swp', 'mem'):
            if key in lsload_dic and idx < len(lsload_dic[key]):
                val = lsload_dic[key][idx]
    ...
    for line in stdout.decode('utf-8', 'ignore').split('\n'):
        ...
        if line:
            if job_compile_dic['job_compile'].match(line):
                if re.match(r'Job <' + re.escape(str(job)) + r'> is not found', line):
                    continue
            ...
            elif job and (not my_dic[job]['rusage_mem']) and job_compile_dic['effective_rusage_mem_compile'].match(line):
...
def get_exec_host_list(exec_host_string):
    """
    Parse an LSF EXEC_HOST string into a de-duplicated list of host names.

    Handles "hostA", "2*hostA", "hostA hostB", "1*hostA:2*hostB", "hostA:hostB".
    The "N*" slot multiplier is stripped. Empty/None returns [].
    """
    if not exec_host_string:
        return []

    host_list = []

    for token in re.split(r'[\s,:]+', str(exec_host_string).strip()):
        if not token:
            continue

        host = re.sub(r'^\d+\*', '', token)
...
def get_bhist_info(command='bhist -l', timeout=120):
    """Get job history with command "bhist -l <jobid>".
    bjobs -UF only sees jobs still in mbatchd memory; completed jobs need
    bhist. Output format (LSF standard):
    ====
    Summary of time in seconds spent in various states:
    JOBID  USER  JOB_NAME  QUEUE  WAIT  CPU_TIME  FINISH_TIME  EXIT_CODE
    ...

    Job <101>, Job Name <..>, User <..>, Project <..>, Queue <..>, Command <..>
    Mon Oct 26 17:43:07: Submitted from host <..>;
    Mon Oct 26 17:43:07: Dispatched .. on Host(s) <..>;
    Mon Oct 26 17:46:17: Done successfully;
    ====
    Returns a dict (single job). On timeout/error returns {'error': .., 'raw': ..}.
    """
    bhist_compile_dic = {'job_compile': re.compile(r'.*Job <([0-9]+(\[[0-9]+\])?)>.*'),
                         'job_name_compile': re.compile(r'.*Job Name <([^>]+)>.*'),
                         'user_compile': re.compile(r'.*User <([^>]+)>.*'),
                         'project_compile': re.compile(r'.*Project <([^>]+)>.*'),
                         'queue_compile': re.compile(r'.*Queue <([^>]+)>.*'),
                         'command_compile': re.compile(r'.*Command <(.+?\S)>.*$'),
                         'submitted_compile': re.compile(r'(\w+ \w+ \d+ [\d:]+): Submitted from host <([^>]+)>.*'),
                         'dispatched_compile': re.compile(r'(\w+ \w+ \d+ [\d:]+): Dispatched.*on Host\(s\) <([^>]+)>.*'),
                         'done_compile': re.compile(r'(\w+ \w+ \d+ [\d:]+): (Done successfully|Exited with exit code (\d+)|Termination request issued).*'),
                         'exit_code_compile': re.compile(r'Exited with exit code (\d+)'),
                         'cpu_time_compile': re.compile(r'.*The CPU time used is (\d+(\.\d+)?) seconds.*'),
                         'max_mem_compile': re.compile(r'.*MAX MEM: (\d+(\.\d+)?)\s*([KMGT]bytes).*'),
                         'avg_mem_compile': re.compile(r'.*AVG MEM: (\d+(\.\d+)?)\s*([KMGT]bytes).*')}

    my_dic = {}
    (return_code, stdout, stderr) = common.run_command(command, timeout=timeout)

    if return_code != 0 and not stdout:
        error_str = str(stderr, 'utf-8', 'ignore') if stderr else f'bhist failed (return code {return_code})'

        if 'timed out' in error_str:
            error_str = f'bhist timed out after {timeout}s'

        return {'error': error_str, 'raw': ''}

    raw_text = stdout.decode('utf-8', 'ignore')
    events = []
    ...
    for line in raw_text.split('\n'):
        ...
        # Event流水行: "<Month> <Day> <HH:MM:SS>: <desc>"
        event_match = re.match(r'(\w{3} \w{3}\s+\d+ [\d:]+): (.+)', line)

        if event_match:
            events.append({'time': event_match.group(1), 'event': event_match.group(2)})

        for key, pattern in bhist_compile_dic.items():
            ...
            if key == 'job_compile':
                my_dic['job'] = match.group(1)
            elif key == 'job_name_compile':
                my_dic['job_name'] = match.group(1)
            elif key == 'user_compile':
                my_dic['user'] = match.group(1)
            elif key == 'project_compile':
                my_dic['project'] = match.group(1)
            elif key == 'queue_compile':
                my_dic['queue'] = match.group(1)
            elif key == 'command_compile':
                my_dic['command'] = match.group(1)
            elif key == 'submitted_compile':
                my_dic['submitted_time'] = match.group(1)
                my_dic['submitted_from'] = match.group(2)
            elif key == 'dispatched_compile':
                my_dic['started_time'] = match.group(1)
                my_dic['started_on'] = match.group(2)
            elif key == 'done_compile':
                my_dic['finished_time'] = match.group(1)
                my_dic['status'] = match.group(2)

                if match.group(4):
                    my_dic['exit_code'] = match.group(4)
            elif key == 'cpu_time_compile':
                my_dic['cpu_time'] = match.group(1)
            elif key == 'max_mem_compile':
                my_dic['max_mem'] = match.group(1) + ' ' + match.group(3)
            elif key == 'avg_mem_compile':
                my_dic['avg_mem'] = match.group(1) + ' ' + match.group(3)

    if not my_dic and not events:
        return {'error': 'No history found (job may not exist).', 'raw': raw_text}

    my_dic['events'] = events
...
def get_bout_info(jobid, timeout=15):
    """Get running job stdout via "bpeek <jobid>".
    Output is raw log text (no structure to parse). Returns:
    {'jobid': .., 'output': .., 'error': .. (if any)}.
    """
    command = 'bpeek ' + str(jobid)
    (return_code, stdout, stderr) = common.run_command(command, timeout=timeout)
    output = stdout.decode('utf-8', 'ignore') if stdout else ''
    result = {'jobid': str(jobid), 'output': output}

    if return_code != 0:
        error_str = str(stderr, 'utf-8', 'ignore') if stderr else f'bpeek failed (return code {return_code})'

        if 'timed out' in error_str:
            error_str = f'bpeek timed out after {timeout}s'
