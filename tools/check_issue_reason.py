...
def read_args():
    ...
    parser = argparse.ArgumentParser(
        description="""
check_issue_reason — LSF 作业问题诊断工具

图形化诊断 LSF 作业的 PEND/SLOW/FAIL 问题，显示原因分析和处理建议。
""",
...
class MainWindow(QMainWindow):
    ...
    def get_exit_code_info(self):
        """
        Get exit_code&exit_reason from config/lsf/exit_code.yaml.
        """
        exit_code_dic = {}
        exit_code_file = str(os.environ['LSFMONITOR_INSTALL_PATH']) + '/config/lsf/exit_code.yaml'
    ...
    def get_term_signal_info(self):
        """
        Get term_signal&term_reason from config/lsf/term_signal.yaml.
        """
        term_signal_dic = {}
        term_signal_file = str(os.environ['LSFMONITOR_INSTALL_PATH']) + '/config/lsf/term_signal.yaml'
    ...
    def gen_select_frame(self):
        ...
        empty_label_1 = QLabel(self.select_frame)
        empty_label_2 = QLabel(self.select_frame)
    ...
    def check_fail_issue(self, job, job_dic):
        ...
        if job_dic[job]['status'] == 'DONE':
            self.info_text.append('Job done successfully.')
...
class ProcessTracer(QThread):
    ...
    def run(self):
        command = [str(os.environ['LSFMONITOR_INSTALL_PATH']) + '/tools/process_tracer', '-j', str(self.job)]
