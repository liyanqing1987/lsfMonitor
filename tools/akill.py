...
按 jobid/job_name/command/submit_time/execute_host/queue/user 等条件批量删除 LSF 作业，
支持模糊匹配和 jobid 范围（如 "10200-10450"）。
""",
    ...
    parser.add_argument('-j', '--jobid',
                        nargs='+',
                        default=[],
                        help='按 jobid 删除作业，支持模糊匹配，也支持 jobid 范围如 "10200-10450"')
    parser.add_argument('-J', '--job_name',
                        nargs='+',
                        default=[],
                        help='按 job_name 删除作业，支持模糊匹配')
    parser.add_argument('-c', '--command',
                        nargs='+',
                        default=[],
                        help='按 command 删除作业，支持模糊匹配')
    parser.add_argument('-s', '--submit_time',
                        nargs='+',
                        default=[],
                        help='按 submit_time 删除作业，支持模糊匹配')
...
class AutoKill():
    ...
    def get_real_jobid_list(self):
        orig_jobid_list = list(self.jobid_list)
        self.jobid_list = []
        ...
        for jobid in orig_jobid_list:
            if jobid == '0':
                ...
                self.jobid_list.append(jobid)
            elif my_match := re.match(r'^(\d+)-(\d+)$', jobid):
    ...
    def kill_base_execute_host(self):
        for execute_host in self.execute_host_list:
            command = 'bkill -m ' + shlex.quote(str(execute_host)) + ' 0'
            self.run_command(command)

    def kill_base_queue(self):
        for queue in self.queue_list:
            command = 'bkill -q ' + shlex.quote(str(queue)) + ' 0'
            self.run_command(command)

    def kill_base_user(self):
        for user in self.user_list:
            command = 'bkill -u ' + shlex.quote(str(user)) + ' 0'
            self.run_command(command)
