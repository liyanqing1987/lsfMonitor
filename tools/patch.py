...
def read_args():
    ...
    parser = argparse.ArgumentParser(
        description="""
patch — lsfMonitor 升级/补丁工具

将新安装包的文件同步到现有安装目录，支持配置迁移、shell wrapper 重新生成和备份。
""",
...
class Patch():
    def __init__(self, patch_path, dry_run=False, no_backup=False):
        ...
        # Config files handled via migration, not direct copy.
        self.config_rel_paths = [
    ...
    def is_excluded(self, rel_path):
        ...
        # Exclude config files (handled via migration).
        if rel_path in self.config_rel_paths:
            return True
    ...
    def display_summary(self, new_files, modified_files, deleted_files):
        ...
        for config_rel_path in self.config_rel_paths:
            new_config = os.path.join(self.patch_path, config_rel_path)
            old_config = os.path.join(self.install_path, config_rel_path)
            ...
            if os.path.exists(new_config) and os.path.exists(old_config):
                if not filecmp.cmp(old_config, new_config, shallow=False):
                    ...
                    common.bprint(f'  {config_rel_path} will be migrated (user values preserved, new variables added).', color='cyan')
                    common.bprint('')
    ...
    def create_backup(self, modified_files, deleted_files):
        ...
        for config_rel_path in self.config_rel_paths:
            config_src = os.path.join(self.install_path, config_rel_path)

            if os.path.exists(config_src):
                config_dst = os.path.join(backup_dir, config_rel_path)
                os.makedirs(os.path.dirname(config_dst), exist_ok=True)
                shutil.copy2(config_src, config_dst)
                config_backup_count += 1

        common.bprint(f'    Backed up {len(files_to_backup)} file(s) + {config_backup_count} config file(s)')
        common.bprint('')
    ...
    def migrate_config(self):
        """
        Migrate config files: preserve user values, add new variables from template.
        Iterates over all config/config_*.py files (lsf / license / run / ai).
        """
        for config_rel_path in self.config_rel_paths:
            self._migrate_single_config(config_rel_path)

    def _migrate_single_config(self, config_rel_path):
        """
        Migrate one config file: preserve user values, add new variables from template.
        """
        old_config_path = os.path.join(self.install_path, config_rel_path)
        new_config_path = os.path.join(self.patch_path, config_rel_path)
        ...
        if not os.path.exists(old_config_path):
            ...
            common.bprint(f'>>> {config_rel_path}: copied new config (no existing config found).')
            return

        if filecmp.cmp(old_config_path, new_config_path, shallow=False):
            common.bprint(f'>>> {config_rel_path}: no changes needed.')
            return

        common.bprint(f'>>> Migrating config file "{config_rel_path}"')
        ...
        os.chmod(old_config_path, 0o644)
...
# Set install path.
export LSFMONITOR_INSTALL_PATH={shlex.quote(self.install_path)}
...
# Capture the user's original command line for the launch log.
