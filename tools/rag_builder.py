...
def read_args():
    ...
    parser = argparse.ArgumentParser(
        description='rag_builder — RAG 向量库构建/更新工具\n\n从 PDF、文本、Markdown、reStructuredText 文件构建或更新 RAG 向量数据库\n（rag_chunks.json + rag_faiss.index），供 AI 助手检索使用。',
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""\
使用示例：

  # 从文件构建（追加模式，跳过已索引的文件）
  rag_builder -i /path/to/docs/

  # 从多个文件/目录构建
  rag_builder -i file1.pdf file2.txt /path/to/dir/

  # 丢弃已有数据，全量重建
  rag_builder -i /path/to/docs/ --rebuild

  # 列出已索引的文档
  rag_builder -l

  # 删除匹配的文档（按文件名子串匹配）
  rag_builder -d old_report.pdf

  # 使用压缩索引（节省空间，略有精度损失）
  rag_builder -i /path/to/docs/ --rebuild --compress sq8
""")
    parser.add_argument('-i', '--input_files',
                        nargs='+',
                        default=[],
                        help='输入文件或目录（目录会递归扫描 .pdf/.txt/.md/.rst 文件）')
    parser.add_argument('-l', '--list',
                        action='store_true',
                        default=False,
                        help='列出 RAG 数据库中已索引的全部文档')
    parser.add_argument('-d', '--delete',
                        nargs='+',
                        default=[],
                        help='从 RAG 数据库中删除文档（按文件名子串匹配）')
    parser.add_argument('--rebuild',
                        action='store_true',
                        default=False,
                        help='丢弃已有数据并从头重建（默认：追加模式）')
    ...
    parser.add_argument('-o', '--output_dir',
                        default='',
                        help='RAG 文件输出目录（默认：<config_ai.db_path 或 config.db_path/ai>/rag）')
    parser.add_argument('--prefix',
                        default='rag',
                        help='输出文件名前缀（默认：rag，生成 rag_chunks.json 等）')
    ...
    parser.add_argument('--batch_size',
                        type=int,
                        default=10,
                        help='每次 embedding API 调用的分块数量（默认：10）')
    parser.add_argument('--workers',
                        type=int,
                        default=10,
                        help='embedding API 调用的并发工作线程数（默认：10）')
...
class RagBuilder():
    def __init__(self, args):
        ...
        # Default output dir: config_ai.db_path/rag, or config.py db_path/ai/rag.
        if args.output_dir:
            self.output_dir = args.output_dir
        else:
            self.output_dir = os.path.join(common_db_path.resolve_db_path(config_ai, 'ai'), 'rag')
        ...
        # Resolve embedding API config_ai (fall back to main AI config_ai if embedding-specific is empty).
        self.api_base_url = getattr(config_ai, 'ai_embedding_api_base_url', '') or getattr(config_ai, 'ai_api_base_url', '')
        self.api_key = getattr(config_ai, 'ai_embedding_api_key', '') or getattr(config_ai, 'ai_api_key', '')
        self.embedding_model = getattr(config_ai, 'ai_embedding_model_name', '')
    ...
    def load_existing_data(self):
        ...
        if os.path.exists(self.embeddings_file):
            try:
                ...
                # .npy may be saved as float16 to halve disk size; convert back to float32 for FAISS compatibility on load.
    ...
    def get_embeddings(self, texts):
        ...
        if not self.api_base_url or not self.api_key or not self.embedding_model:
            common.bprint('Embedding API not configured. Set ai_embedding_* or ai_api_* in config_ai.', level='Error')
            sys.exit(1)
        ...
        # requests.Session is NOT thread-safe: concurrent post() from multiple
        # workers can corrupt the connection pool. Probe runs in the main thread
        # (safe to reuse `session`); worker threads instead each get their own
        # Session via this thread-local, so connections are never shared across
        # threads.
        import threading
        ...
        if use_batch:
            ...
            def _batch_worker(batch_info):
                _, s, e = batch_info
                return (s, self._get_embeddings_batch(texts[s:e], url, headers, _worker_session()))
            ...
            def _single_worker(idx_text):
                idx, text = idx_text
                return (idx, self._get_embedding_single(text, url, headers, _worker_session()))
    ...
    def save(self, chunks, metadata, embeddings, faiss_index):
        ...
        faiss_index / embeddings may be None when delete_sources shrinks the
        DB to chunks-only (no embeddings to rebuild an index from). In that
        case the index/embeddings files are removed so the on-disk state stays
        consistent with the chunks-only data instead of going stale.
        """
        import faiss

        # Create output dir as 0o1777 (shared write + sticky) so multiple
        # users can write RAG files; standard files are then 0o644 so a
        # non-owner cannot overwrite the first builder's files (sticky also
        # blocks delete). Golden spec: docs/db_permissions.md.
        common.create_dir(self.output_dir, 0o1777)

        with open(self.chunks_file, 'w') as f:
            json.dump(chunks, f, ensure_ascii=False, indent=2)

        os.chmod(self.chunks_file, 0o644)
        common.bprint(f'  Saved {self.chunks_file} ({len(chunks)} chunks)')

        if faiss_index is not None:
            faiss.write_index(faiss_index, self.faiss_file)
            os.chmod(self.faiss_file, 0o644)
            common.bprint(f'  Saved {self.faiss_file}')
        elif os.path.exists(self.faiss_file):
            os.remove(self.faiss_file)
            common.bprint(f'  Removed stale {self.faiss_file} (no index to write)')

        with open(self.metadata_file, 'w') as f:
            json.dump(metadata, f, ensure_ascii=False, indent=2)

        os.chmod(self.metadata_file, 0o644)
        common.bprint(f'  Saved {self.metadata_file}')

        if embeddings is not None:
            np.save(self.embeddings_file, embeddings.astype(np.float16))
            os.chmod(self.embeddings_file, 0o644)
            emb_size_mb = os.path.getsize(self.embeddings_file) / 1024 / 1024
            common.bprint(f'  Saved {self.embeddings_file} (float16, {emb_size_mb:.1f} MB)')
            common.bprint(f'  Note: {os.path.basename(self.embeddings_file)} is only needed for --append (incremental update). If you always use --rebuild, you can safely delete it to save disk space.')
        elif os.path.exists(self.embeddings_file):
            os.remove(self.embeddings_file)
            common.bprint(f'  Removed stale {self.embeddings_file} (no embeddings)')
    ...
    def delete_sources(self, patterns):
        ...
        if new_embeddings is None:
            common.bprint('*Warning*: No embeddings available to rebuild FAISS index, skipping rebuild.', level='Warning')
            self.save(new_chunks, new_metadata, new_embeddings, None)
...
def main():
    ...
    # Ensure db_path root is 0o1777 before save() writes under db_path/ai/rag.
    # For a custom db_path created on first run, makedirs would otherwise leave
    # the root at 0o755 (umask) and lock out other sampler accounts.
    try:
        ai_root = common_db_path.resolve_db_path(config_ai, 'ai')
        common_db_path.ensure_db_root(os.path.dirname(ai_root))
    except Exception:
        pass
