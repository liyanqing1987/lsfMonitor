...
| `ai_forbidden_commands` | AI 禁止执行的命令（空格分隔） |
| `ai_dangerous_commands` | 需用户确认的危险命令（空格分隔，默认 `bkill badmin brestart bstop bresume bswitch`） |
...
| `-m` | 采集运行中作业内存用量 | `bjobs -u all -r -UF` |
| `-q` | 采集队列信息 | `bqueues` |
...
*/5 * * * * /path/to/monitor/bin/bsample -m        # 作业内存
